...
const DASHED_PATHS = ['M64 195L133 195', PARSER_TO_TEMPLATE_PATH, ...TEMPLATE_TO_PANEL_PATHS];
...
function HowAnimation({ className, ...restProps }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  const mpathTemplateToPanel = TEMPLATE_TO_PANEL_PATHS.map((_, i) => `${id}TemplateToPanel${i}`);
  const mpathPanelToIndexer = PANEL_TO_INDEXER_PATHS.map((_, i) => `${id}PanelToIndexer${i}`);
  ...
  const parseToParserDotBegin = offsetBegin(cogBegin, PARSE_TO_PARSER_DOT_DELAY);
  ...
  const templateDotBegin = offsetBegin(templateHighlightBegin, TEMPLATE_DOT_DELAY);
      ...
      >
        <defs>
          ...
          {TEMPLATE_TO_PANEL_PATHS.map((d, i) => (
            <path key={mpathTemplateToPanel[i]} id={mpathTemplateToPanel[i]} d={d} />
          ))}
          {PANEL_TO_INDEXER_PATHS.map((d, i) => (
            <path key={mpathPanelToIndexer[i]} id={mpathPanelToIndexer[i]} d={d} />
          ))}
          ...
          <SvgGlowFilter id={parseIconGlowFilter} stdDeviation={3} extrude={0} />
        </defs>
        ...
        {!shouldReduceMotion && (
          <>
            ...
            {PANEL_DEFS.map((_, i) => (
              <use key={`template-dot-${i}`} href={`#${shapeDot}`} fill='#42FFA4' opacity='0'>
                <animate
                  attributeName='opacity'
                  values='0;1;1;0'
                  keyTimes='0;0.1;0.9;1'
            ...
            {PANEL_DEFS.map((_, i) => {
              const panelDotBegin = offsetBegin(panelHighlightBegins[i], PANEL_DOT_DELAY);
              return (
